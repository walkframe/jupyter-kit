...
Vue / Svelte / Web Component / vanilla variants + every opt-in (math,
executor, widgets, …) are generated by the
...
- **Remote Jupyter kernels** — `@jupyter-kit/executor-jupyter` talks to
...
| `@jupyter-kit/react` / `vue` / `svelte` / `wc` | Framework wrappers (`<Notebook>` / `<jk-notebook>`) |
...
| `@jupyter-kit/executor-jupyter` | Remote Jupyter Server / Hub / Binder kernel executor |
