...
| \+ Edit + run Python | <InstallLine label="" command="pnpm add @jupyter-kit/executor-pyodide" /> |
| \+ Edit + run R (GPL-2.0+) | <InstallLine label="" command="pnpm add @jupyter-kit/executor-webr webr" /> |
...
| `@jupyter-kit/react` / `vue` / `wc` | +5 KB | Thin wrapper around core |
...
|---|---|
| `@jupyter-kit/react` / `vue` / `wc` | `@jupyter-kit/core` |
...
### Edit + execute in browser

Installing an executor gives you editable cells **and** the matching
runtime in one command — the editor plugin is pulled in transitively:

| Install | Runs | Auto-installs |
|---|---|---|
| `@jupyter-kit/executor-pyodide` | Python via Pyodide (CDN) | `editor-codemirror` + CodeMirror + Python grammar + `comm` |
| `@jupyter-kit/executor-webr webr` | R via WebR (CDN) | `editor-codemirror` + CodeMirror + R grammar |
